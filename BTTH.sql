...
CREATE PROCEDURE ProcessEquipmentPurchase (
    ...
    OUT p_annouce VARCHAR (100)
)
BEGIN
	DECLARE p_status VARCHAR (20);
    DECLARE p_stock INT;
    
    SELECT stock INTO p_stock FROM products WHERE p_item_id = product_id;
    SELECT status INTO p_status FROM wallets WHERE patient_id = p_patient_id;
    
    IF p_status = 'Inactive' THEN SET p_annouce = 'Thất bại: Ví đang khoá ';
	END IF;
    
    IF p_stock < p_stock_quantity AND p_status = 'Active' THEN 
    SET p_annouce = 'Thất bại: Số lượng còn lại không đủ';
    ELSEIF p_stock >= p_stock_quantity AND p_status = 'Active' THEN 
    UPDATE Products SET stock = stock - p_stock_quantity WHERE p_item_id = product_id;
    END IF;
END
// DELIMITER ;
SET @abc = '';
CALL ProcessEquipmentPurchase (1,1,100,@abc); 
SELECT @abc ;
-- !!! theo từng case
